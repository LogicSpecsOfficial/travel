...
export default function App() {
  ...
  const handleDurationChange = (val) => {
      ...
      if (hasPointsInDeletedDays) {
          setDeleteConfirmation({
              ...
              id: newCount,
              type: 'duration',
              message: `Reducing to ${newCount} days will permanently delete locations on days ${newCount + 1} through ${currentTrip.dayCount}.`
          });
      } else {
        ...
        dayPoints.forEach((point, i) => {
          if (i < dayPoints.length - 1) {
            ...
            if (element.status === "OK") {
              const realIndex = updatedPoints.findIndex(p => p.id === point.id);
              updatedPoints[realIndex] = {
                ...point,
                travelData: {
                  toId: dayPoints[i + 1].id,
                  mode: 'DRIVING',
  ...
  function checkOpeningStatus(arrivalDate, opening_hours) {
    ...
    for (const period of todaysPeriods) {
        ...
        if (arrivalTime >= openTime && arrivalTime < closeTime) {
            return { status: 'open', message: `Open until ${period.close?.time.substring(0, 2)}:${period.close?.time.substring(2)}` };
        }
    }
    return { status: 'closed', message: `Closed during arrival` };
  }
  ...
  const addPointFromLibrary = (loc) => {
    const newPoint = {
        ...
        stayMinutes: 60, travelMode: 'DRIVING', day: selectedDay, opening_hours: loc.opening_hours
    };
    const updatedPoints = [...currentTrip.points, newPoint];
    setCurrentTrip(prev => ({ ...prev, points: updatedPoints }));
    updateTravelTimes(updatedPoints);
